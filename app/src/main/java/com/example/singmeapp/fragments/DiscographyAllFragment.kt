        ...
        when(whatIs){
            "tracks" -> {
                discographyAllViewModel.listTrack.observe(viewLifecycleOwner){
                    tracksAdapter.trackList = it as ArrayList<Track> /* = java.util.ArrayList<com.example.singmeapp.items.Track> */
                    binding.rcDiscographyAll.adapter = tracksAdapter
                }
            }
            "albums" -> {
                discographyAllViewModel.listAlbum.observe(viewLifecycleOwner){
                    albumsAdapter.albumList = it as ArrayList<Album> /* = java.util.ArrayList<com.example.singmeapp.items.Album> */
                    binding.rcDiscographyAll.adapter = albumsAdapter
                }
            }
            "singles" -> {
                discographyAllViewModel.listSingle.observe(viewLifecycleOwner){
                    singlesAdapter.albumList = it as ArrayList<Album> /* = java.util.ArrayList<com.example.singmeapp.items.Album> */
